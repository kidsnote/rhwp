...
impl LayoutEngine {
    ...
    fn build_single_column(
        ...
        for (item_ordinal, item) in col_content.items.iter().enumerate() {
            ...
            // 하단 밖(재현 문서 C Enter 8회: y=1049 시작, 297px 넘침)에 그려진다.
            ...
            // 만들어(재현 문서 C: lh 1400 · ls -700 → 상자 9.3px, 글자 18.7px),
            // 흐름 커서가 글자 하단보다 위에 머문다. 편집으로 표가 이 쪽에
            // 재배치되면 그 좁은 상자 위치에 표를 그려 앞 문구를 문다(문구
            // 146~165, 표 157). 한글은 재조판에서 표를 글자 아래에 놓는다.
        ...
        if let Some(Control::Table(t)) = para.controls.get(control_index) {
            ...
            let fitted_visible_mt = if is_current_visible_para_float {
                raw_mt.map(|measured| {
                    ...
                    // 성장) 압축하지 않는다(재현 문서 A 셀 Enter: 압축하면 커진
                    // 행의 몫을 다른 행이 빼앗겨 내부가 위로 밀린다).
                    if self.profile.get().session_edited()
    ...
    fn layout_table_item(
        ...
        if let Some(para) = paragraphs.get(para_index) {
            ...
            // 오인해 이 쪽에 한 번 더 그리면 문구·로고가 두 쪽에 중복된다(재현 문서 A
            // 셀 끝 Enter 3~4회). 편집 세션은 PP 아이템 배정이 진실이므로 끈다.
                ...
                if let Control::Picture(pic) = ctrl {
                    if pic.common.treat_as_char {
                        ...
                        if is_paper_based {
                            ...
                            // 올리면 커진 표 하단에 겹친다(재현 문서 C Enter 8회:
                            // 벌 그림이 표 끝 1027px 위 1004px 에 그려짐).
